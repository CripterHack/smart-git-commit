...
class SmartGitCommitWorkflow:
    ...
    def _run_git_command(self, args: List[str]) -> Tuple[str, int]:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.warning(f"Git command failed with code {process.returncode}: git {' '.join(args)}")
    ...
    def load_changes(self) -> None:
        ...
        try:
            with Spinner(message="Getting changes from git repository...", spinner_type=2) as spinner:
                ...
                # Use standard porcelain format without -z for more reliable parsing
                stdout, code = self._run_git_command(["status", "--porcelain"])
                
                # Debug log the raw output
                logger.debug(f"Raw git status output: {repr(stdout)}")
                ...
                # Parse the standard porcelain format
                entries = []
                if stdout:
                    # Split by newlines and process each line
                    lines = stdout.splitlines()
                    logger.debug(f"Split into {len(lines)} status lines")
    ...
    def _analyze_batch_parallel(self, batch: List[GitChange]) -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
            ...
            try:
                concurrent.futures.wait(futures, timeout=60)  # 60-second overall timeout
            except Exception as e:
                logger.warning(f"Error waiting for thread pool: {str(e)}")
    ...
    def execute_commits(self, interactive: bool = True) -> None:
        ...
        try:
            ...
            # Check for pre-commit hooks and module availability
            has_precommit_hooks = self._check_for_precommit_hooks()
            precommit_available = self._is_precommit_module_available() if has_precommit_hooks else False
            
            # Auto-detect if we should skip hooks based on environment
            auto_skip_hooks = False
            if has_precommit_hooks and not precommit_available and not self.skip_hooks:
                logger.warning("Pre-commit hooks detected but pre_commit module is not available in your environment.")
                logger.warning("Smart Git Commit will automatically skip pre-commit hooks to proceed.")
                logger.warning("To install pre-commit: pip install pre-commit")
                print(f"\n{Colors.YELLOW}⚠️  Pre-commit hooks detected but pre_commit module is not installed.{Colors.RESET}")
                print(f"{Colors.CYAN}⚙️  Smart Git Commit will automatically skip pre-commit hooks to proceed.{Colors.RESET}")
                print(f"{Colors.GREEN}💡 Tip: Install pre-commit with: pip install pre-commit{Colors.RESET}\n")
                self.skip_hooks = True
                auto_skip_hooks = True
                
            print(f"\n{Colors.BLUE}💾 Executing commits...{Colors.RESET}")
            successful_commits = 0
            
            # Keep track of total commits for summary
            ...
            if name_exit != 0 or email_exit != 0 or not name_out.strip() or not email_out.strip():
                print(f"\n{Colors.YELLOW}⚠️  Git user configuration not properly set. Please configure git with:{Colors.RESET}")
                print(f"  {Colors.CYAN}git config --global user.name \"Your Name\"{Colors.RESET}")
                print(f"  {Colors.CYAN}git config --global user.email \"your.email@example.com\"{Colors.RESET}")
                if interactive and input(f"\nContinue anyway? [y/N]: ").lower() != 'y':
                    return
            
            # Loop through each commit group
            for i, group in enumerate(self.commit_groups):
                logger.info(f"Commit {i+1}/{total_commits}: {group.name} ({group.file_count} files)")
                
                # Verify group has changes to commit
                if not group.changes:
                    logger.warning(f"Skipping empty group: {group.name}")
                    continue
                
                # Log the files in the commit group
                ...
                print(f"\n{Colors.BOLD}Commit {i+1}/{total_commits}: {Colors.GREEN}{group.name}{Colors.RESET}")
                print(f"{Colors.BLUE}Files: {Colors.RESET}{', '.join(os.path.basename(f) for f in file_list[:3])}" + 
                ...
                if interactive:
                    ...
                    if choice == "" or choice == "y":
                        ...
                        print(f"{Colors.YELLOW}Skipping this commit group.{Colors.RESET}")
                        continue
                    elif choice == 'e':
                        # Allow editing the commit details
                        print(f"\n{Colors.BOLD}{Colors.BLUE}Edit commit details:{Colors.RESET}")
                        new_name = input(f"Commit name [{Colors.GREEN}{group.name}{Colors.RESET}]: ").strip()
                        if new_name:
                            group.name = new_name
                        ...
                        for j, ct in enumerate(commit_types):
                            print(f"  {Colors.CYAN}{j+1}. {ct.value}: {ct.name}{Colors.RESET}")
                            
                        try:
                            type_choice = int(input(f"\n{Colors.BOLD}Commit type [{commit_types.index(group.commit_type)+1}]: {Colors.RESET}") or (commit_types.index(group.commit_type)+1))
                            if 1 <= type_choice <= len(commit_types):
                                group.commit_type = commit_types[type_choice-1]
                        except ValueError:
                            pass  # Keep the original commit type
                            
                        # Allow editing the description
                        print(f"\n{Colors.BOLD}Enter a detailed description for this commit (empty line to finish):{Colors.RESET}")
                        lines = []
                        while True:
                            line = input()
                            if not line:
                        ...
                        # Allow adding issue references
                        issue_input = input(f"\n{Colors.BOLD}Reference issues (comma-separated, e.g. '123, 456'): {Colors.RESET}")
                        if issue_input.strip():
                            issue_numbers = [issue.strip() for issue in issue_input.split(",")]
                            for issue in issue_numbers:
                                if issue.isdigit():
                                    group.issues.add(issue)
                
                # Stage the files for this commit - individually
                logger.info("Staging files individually:")

                # Create a progress spinner for file staging
                total_files = len(group.changes)
                staged_files = []  # Keep track of successfully staged files
                
                with Spinner(message=f"Staging {total_files} files...", spinner_type=0, show_progress_bar=True, total=total_files) as spinner:
                    for idx, change in enumerate(group.changes):
                        ...
                        try:
                            # Use -- to separate options from file arguments (important for files with dashes)
                            out, exit_code = self._run_git_command(["add", "--", norm_path])
                            
                            if exit_code != 0:
                                # Try fallback to absolute path if relative fails
                                abs_path = os.path.abspath(os.path.join(self.repo_path, norm_path))
                                out, exit_code = self._run_git_command(["add", "--", abs_path])
                                
                                if exit_code != 0:
                                    # Windows compatibility: try with backslashes 
                                    win_path = change.filename.replace('/', '\\')
                                    out, exit_code = self._run_git_command(["add", "--", win_path])
                                    
                                    if exit_code != 0:
                                        logger.error(f"Failed to stage file {norm_path}: {out}")
                                        spinner.stop()
                                        if not interactive or input(f"\n{Colors.RED}Continue despite staging error for {norm_path}? [y/N]: {Colors.RESET}").lower() != 'y':
                                            raise RuntimeError(f"Failed to stage changes for {norm_path}")
                            
                            if exit_code == 0:
                                staged_files.append(norm_path)
                                logger.info(f"Successfully staged: {norm_path}")
                        except Exception as e:
                            logger.error(f"Error staging file {norm_path}: {str(e)}")
                            spinner.stop()
                            if not interactive or input(f"\n{Colors.RED}Continue despite error for {norm_path}? [y/N]: {Colors.RESET}").lower() != 'y':
                                raise RuntimeError(f"Error staging file {norm_path}: {str(e)}")
                    
                    # Verify all files were staged successfully
                    if len(staged_files) < len(group.changes):
                        missing_files = [change.filename for change in group.changes if change.filename not in staged_files]
                        logger.warning(f"Failed to stage some files: {missing_files}")
                
                # Prevent commit if no files were staged
                if not staged_files:
                    logger.error("No files were staged successfully. Aborting commit.")
                    spinner.stop()
                    print(f"\n{Colors.RED}❌ ERROR: No files were staged successfully. Aborting commit.{Colors.RESET}")
                    continue
                
                # Verify staging status with git status
                logger.info("Verifying staged files...")
                staged_status, _ = self._run_git_command(["status", "--porcelain"])
                staged_count = sum(1 for line in staged_status.splitlines() if line.startswith("A ") or line.startswith("M "))
                
                if staged_count == 0:
                    logger.error("No files appear to be staged according to git status. Staging may have failed.")
                    if interactive:
                        if input(f"\n{Colors.YELLOW}⚠️ No files appear to be staged. Retry staging? [Y/n]: {Colors.RESET}").lower() != 'n':
                            # Try staging files one more time with a different approach
                            print(f"\n{Colors.CYAN}Retrying staging with alternative method...{Colors.RESET}")
                            
                            # Try a simpler staging approach, one by one
                            for file_path in [change.filename for change in group.changes]:
                                file_path = file_path.strip()
                                # Escape spaces and special characters
                                safe_path = file_path.replace('"', '\\"')
                                _, status = self._run_git_command(["add", safe_path])
                                if status == 0:
                                    logger.info(f"Re-staged file: {file_path}")
                            ...
                            if staged_count == 0:
                                logger.error("Still no files staged after retry. Abandoning commit.")
                                print(f"\n{Colors.RED}❌ ERROR: Still unable to stage files. Aborting commit.{Colors.RESET}")
                                continue
                        else:
                            logger.info("User chose not to retry staging. Continuing with commit attempt.")
                
                # Generate the commit message
                commit_message = group.generate_commit_message()
                
                # Path for the commit message file
                git_dir = self._get_git_root()
                commit_msg_file = os.path.join(git_dir, ".git", "COMMIT_EDITMSG")
                ...
                commit_cmd = ["commit", "--file", commit_msg_file]
                if self.skip_hooks:
                ...
                if exit_code == 0:
                    ...
                    print(f"{Colors.GREEN}✅ Committed: {group.commit_type.value}({', '.join(sorted({change.component for change in group.changes if hasattr(change, 'component')})[:2])}): {group.name}{Colors.RESET}")
                else:
                    logger.warning(f"Git command failed: {out}")
                    print(f"{Colors.RED}❌ Commit failed: {out}{Colors.RESET}")
                    
                    # Check specifically for pre-commit module errors
                    if "No module named pre_commit" in out or "ModuleNotFoundError: No module named 'pre_commit'" in out:
                        print(f"\n{Colors.YELLOW}⚠️  Pre-commit hook error detected. Trying again with --no-verify...{Colors.RESET}")
                        # Try again with --no-verify
                        commit_cmd = ["commit", "--file", commit_msg_file, "--no-verify"]
                        out, exit_code = self._run_git_command(commit_cmd)
                        if exit_code == 0:
                            successful_commits += 1
                            print(f"{Colors.GREEN}✅ Committed (with hooks disabled): {group.name}{Colors.RESET}")
                        else:
                            logger.error(f"Commit still failed with --no-verify: {out}")
                            print(f"{Colors.RED}❌ Commit failed even with hooks disabled{Colors.RESET}")
                ...
                if os.path.exists(commit_msg_file):
                    try:
                        ...
                        logger.warning(f"Could not remove commit message file: {e}")
            
            # Check if all commits failed
            if successful_commits == 0:
                if auto_skip_hooks:
                    logger.error("All commits failed. This may be due to auto-skipping pre-commit hooks.")
                    print(f"\n{Colors.RED}❌ All commits failed. This may be because pre-commit hooks were auto-skipped.{Colors.RESET}")
                    ...
                    print(f"   2. {Colors.CYAN}Run with explicit hook skipping: smart-git-commit --skip-hooks{Colors.RESET}")
                else:
                    logger.error("No commits were successful. Check the logs for details.")
                error_message = "No commits were successful. Check the logs for details."
            ...
            if auto_skip_hooks:
                ...
                print(f"{Colors.YELLOW}⚠️  Note: Pre-commit hooks were auto-skipped. Consider installing pre-commit.{Colors.RESET}")
